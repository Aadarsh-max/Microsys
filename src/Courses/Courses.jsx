...
const courses = [
  ...
  { 
    ...
    theme: {
      ...
      overlay: "from-green-900/20",
      corner: "from-green-300/30"
    }
  },
  { 
    id: 3, 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-7xl mx-auto">
        {courses.map((course, index) => (
          <Link key={course.id} to={course.path} className="group block">
            ...
            >
              <div className={`absolute inset-0 bg-gradient-to-r from-transparent via-${course.theme.border}/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000 ease-out`}></div>
              ...
              <div className="md:w-2/5 mb-6 md:mb-0 relative overflow-hidden rounded-2xl">
                ...
                />
                <div className={`absolute inset-0 bg-gradient-to-t ${course.theme.overlay} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
              </div>
              
              {/* Text */}
              <div className="md:w-3/5 md:pl-8 flex flex-col justify-center relative z-10">
                <h3 className={`text-3xl font-bold ${course.theme.text} mb-3 group-hover:${course.theme.hoverText} transition-colors duration-500 transform group-hover:translate-x-2`}>
                  {course.name}
                </h3>
                <p className={`text-xl ${course.theme.text} mb-4 font-medium group-hover:${course.theme.hoverText} transition-colors duration-500`}>
                  {course.duration}
                </p>
                <p className={`${course.theme.text} text-base leading-relaxed transform group-hover:translate-x-2 transition-transform duration-500 delay-100`}>
                  Click to know more about {course.name} course.
                </p>
                
                <div className={`mt-4 flex items-center ${course.theme.text} group-hover:${course.theme.hoverText} transition-colors duration-500`}>
              ...
              {/* Corner Accent */}
              <div className={`absolute top-0 right-0 w-20 h-20 bg-gradient-to-br ${course.theme.corner} to-transparent rounded-bl-full transform group-hover:scale-150 transition-transform duration-700`}></div>
            </div>
          </Link>
        ))}
      </div>
